  ...
  return (
    <div className="space-y-4">
      <div className="flex items-baseline gap-2">
        <span className="text-4xl font-bold">{movie.Diem_danh_gia.toFixed(1)}</span>
        <div className="flex gap-1">
          {[...Array(5)].map((_, i) => (
            <StarIcon
              key={i}
              className={`h-5 w-5 ${i < Math.round(movie.Diem_danh_gia / 2) ? "fill-yellow-400 text-yellow-400" : "text-gray-300"}`}
            />
          ))}
        </div>
      </div>
        ...
        {[5, 4, 3, 2, 1].map((rating) => {
          ...
          return (
            <div key={rating} className="flex items-center gap-2">
              <span className="w-12 text-sm text-muted-foreground">{rating} sao</span>
